...
This is a first part of our deep-dive into Postgres logical replication. In this part, we will focus on
...
Our goal with this blog post is to demystify the concept of replica identity and why in certain situations you would
want to alter your replica identity, the consideration to make and performance implications.
...
* Replication slots will monitor and record its own position within the WAL files
* An application will then need both a **replication slot** and **publication** to start consuming changes from logical
  replication.
...
guide [here](https://docs.artie.com/tutorials/tables-without-primary-key-s)), you can also modify the replica identity
to be FULL.
...
The USING INDEX replica identity will capture the columns of the index specified. The index must be unique and specified
...
_For Artie customers, we automatically detect and handle this by adding conditional update clauses to deal with this._
...
One common reason we run into this at Artie is that we'll need the previous values so we can replicate a deleted row to a downstream table that has cluster keys specified.

For example, you may have a table called `accounts`, and you cluster the table in Snowflake by `DATE_TRUNC('day', created_at)`. We will need the `created_at` for the previous row if we want to replicate a delete event.

**Workaround**

If you need `created_at` and don't want to change replica identity, you can add `created_at` to be part of the table's primary keys.
...
* You should expect some increase to your database CPU by choosing FULL replica identity.

By enabling replica identity to be FULL:

1. There will be more information written into WAL files, which may increase your disk usage. That being said, WAL files
   are temporary as they are periodically purged, so increase in disk usage is temporary.
2. More load on the network to propagate WAL to the replicas and subscribers, which may increase your CPU.
